...
$baseDir = 'E:\DVS\Project\Aiagent_Helpme\Helpdesk_KB_Portal'
$msaprPath = "$baseDir\Helpdesk_KB_Portal.msapr"
$msappPath = 'E:\DVS\Project\Aiagent_Helpme\Helpdesk_KB_Portal.msapp'
...
Set(
    varIsKBAdmin,
    Lower(User().Email) in [
        "somchai.p@deves.co.th",
        "kanya.m@deves.co.th",
        "teerapat.ti@deves.co.th",
        "admin@deves.co.th"
    ] || "helpdesk" in Lower(User().Email) || "admin" in Lower(User().Email)
);
Set(
    varUserRole,
...
ClearCollect(
    ...
    Sort(
        Filter(
            Systems,
            IsActive = "Yes" || IsActive = true || IsBlank(IsActive)
        ),
        DisplayOrder,
        SortOrder.Ascending
    )
...
Write-Output "Step 3: Updating Helpdesk_KB_Portal.msapr internal package..."
...
# 3c. Replace References\DataSources.json
...
$zip.Dispose()
Write-Output "Step 3 completed: Helpdesk_KB_Portal.msapr updated."
